...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET' && action === 'magic-verify') {
    ...
    try {
      ...
      if (!record) return res.status(400).json({ error: "Link inválido ou já utilizado.", code: "INVALID_TOKEN" });
      if (record.used) return res.status(400).json({ error: "Este link já foi utilizado. Solicite um novo.", code: "ALREADY_USED" });
    ...
    if (action === 'magic-send') {
      ...
      try {
        if (!isRegister) {
          ...
          if (!existing) {
            return res.status(404).json({ error: "Nenhuma conta encontrada com este e-mail. Crie uma conta primeiro.", code: "USER_NOT_FOUND" });
          }
        } else {
          const existing = await prisma.user.findUnique({ where: { email: normalizedEmail } });
          if (existing) {
            return res.status(409).json({ error: "Este e-mail já está em uso. Faça login em vez de criar uma conta.", code: "USER_ALREADY_EXISTS" });
          }
        }

        const thirtySecondsAgo = new Date(Date.now() - 30 * 1000);
        const recentToken = await prisma.magicToken.findFirst({
          where: { email: normalizedEmail, createdAt: { gte: thirtySecondsAgo } }
        });

        if (recentToken) {
          return res.status(429).json({ error: "Aguarde 30 segundos antes de solicitar um novo link.", code: "COOLDOWN_ACTIVE" });
        }
        ...
        const subject = isRegister ? "Confirme seu cadastro — White Private Credit" : "Seu link de acesso — White Private Credit";
