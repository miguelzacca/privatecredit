...
async function buildContext(userMessage, authedUser) {
  ...
  const userRecord = await prisma.user.findUnique({
    ...
    select: { id: true, name: true, email: true, profile: true, phone: true, creci: true, cpf: true, createdAt: true }
  });
...
`);
  ...
  const wantsMarketplace = msg.includes('marketplace') || msg.includes('disponiveis') || msg.includes('linhas') || msg.includes('credito') || msg.includes('taxa');
  ...
  if (wantsMarketplace) {
    ...
    const totalActive = await prisma.creditLine.count({ where: { status: 'ACTIVE' } });
...
[Existem atualmente ${totalActive} linhas ativas no sistema. Abaixo estão as mais recentes]`);
  ...
  if (isInvestor && (msg.includes('minhas') || msg.includes('capital') || msg.includes('rentabilidade'))) {
...
[Instrução para IA: Diga que a rentabilidade exata e contratos em negociação podem ser vistos na tela de /dashboard/investidor/operacoes]`);
    ...
    myLines.slice(0, 5).forEach((line, i) => {
      parts.push(`- Sua Linha: ${fmt(line.capital)} a ${line.interestRate}%/mês (${line.duration}x). Status: ${line.status}.`);
    });
  }

  // Se for empresa, construtora, etc
  const isCompany = userRecord?.profile === 'empresa' || userRecord?.profile === 'construtora';
  if (isCompany && (msg.includes('solicitacoes') || msg.includes('contratos') || msg.includes('pagamentos'))) {
...
export default async function handler(req, res) {
  ...
  if (action === 'ai-chat') {
    ...
    if (!authedUser) {
      return res.status(401).json({ error: 'Você precisa estar logado para falar com a IA.' });
    }
    ...
    const systemPrompt = `Você é o Especialista Financeiro Inteligente da plataforma "White Private Credit", uma plataforma de crédito privado focada em conectar investidores a empresas/construtoras/corretores.
    ...
    try {
      ...
      if (history.length > 1) {
        conversationContext = history.slice(0, -1)
          .map(m => `${m.role === 'user' ? 'Usuário' : 'Especialista'}: ${m.content}`)
          .join('\n');
      ...
      return res.status(500).json({ error: 'Falha ao processar sua solicitação no modelo de inteligência financeira.' });
    }
  }
